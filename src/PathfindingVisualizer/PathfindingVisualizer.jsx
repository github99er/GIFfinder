...
while (FINISH_NODE_COL == START_NODE_COL && FINISH_NODE_ROW == START_NODE_ROW){
  const FINISH_NODE_ROW =  Math.floor(Math.random() * 20)
  const FINISH_NODE_COL = Math.floor(Math.random() * 50)
}
  ...
  render() {
    ...
    return (
      <>
        ...
        <h1 style={{color:"hotpink"}}> Click on the board to create walls!</h1>
        <Navbar search={() => this.visualizeDijkstra()} reset={() => this.resetGrid()}/>
      </>
    );
  }
}
